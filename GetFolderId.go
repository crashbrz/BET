...
func main() {
	fmt.Println("Burp Get Folder ID")
	var burp_url string
	...
	if os.Args == nil || len(os.Args) < 2 {
		fmt.Println("You must provide at least: The BurpSuite Enterprise URL(-u), and the API Key(-k)")
		fmt.Println("Version:", bgf)
	...
	if *version {
		fmt.Println("Burp Get Folder ID:", bgf)
		fmt.Println("Under the SushiWare license.")
		os.Exit(0)
	}
...
func getid(burp_url string, api_key string) {
	...
	payload := strings.NewReader("{\"query\":\"query GetSiteTree {site_tree{folders {id name}}}\"}")
	...
	for _, info := range result.Data.SiteTree.Folders {
