...
func getid(burp_url string, api_key string) {
	...
	payload := strings.NewReader("{\"query\": \"query GetSiteTree {site_tree {sites {id parent_id name}}}\"}")
	...
	for _, info := range result.Data.SiteTree.Sites {
		fmt.Println("FolderID:"+info.ParentID+ ":SiteID:" + info.ID +":SiteName:"+ info.Name)
	}
}
